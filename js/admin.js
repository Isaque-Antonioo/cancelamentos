...
(function() {
    ...
    window.deleteUserHandler = async function(userId) {
        const user = allUsers.find(u => u.id === userId);
        const name = user ? (user.displayName || 'este usuario') : 'este usuario';
        if (!confirm(`Excluir "${name}" permanentemente? Essa ação não pode ser desfeita.`)) return;

        try {
            const ok = await deleteUser(userId);
            if (!ok) throw new Error('Firebase retornou falha ao excluir.');
            if (typeof logAuditEvent === 'function') {
                logAuditEvent('user_deleted', { targetUserId: userId, displayName: name });
            }
            await loadUsers();
        } catch (error) {
    ...
    function getActionBadge(action) {
        const labels = {
            ...
            'user_deactivated': 'Usuario Desativado', // mantido só pra exibir entradas antigas do audit log
